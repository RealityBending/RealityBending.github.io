...
at the domain root — true in production, false for a server started at the repo
with the site under /WIP/, and false over file://, both of which it breaks
completely (no stylesheets, no modules, a bare page). A relative base is
...
def load_template() -> str:
    ...
    **Asset URLs are deliberately left relative.** An earlier version rewrote
    ...
    mounted at the domain root — true in production, false for a server started
    at the repo with the site under /WIP/, and false over file://. The `<base>`
    each page carries does the same job without the assumption, so there is
