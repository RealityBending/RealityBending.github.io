...
The repository root *is* the site: `index.html` at the top, `.github/workflows/`
beside it, and the deployed site is whatever is checked out here. It replaced a
Hugo/blogdown project that lived at this same root and published to a `gh-pages`
branch; none of that survives, and nothing in the tree below is left over from
it.
...
tools/              developer tools. **Nothing here is part of the site** — no
  ...
  build_legacy_map.py the old Hugo site's URLs → legacy_map.json. **Frozen — it
                      can no longer run**: it joins `docs/sitemap.xml` and
                      `content/` from the old Hugo site, both deleted at the
                      migration. Its output is committed and is what
                      generate_pages.py reads
  legacy_sitemap.xml  the old site's deployed sitemap, 354 URLs, kept because the
                      `gh-pages` branch that held the only copy is going away.
                      legacy_map.json maps 100 of them; this is the only thing
                      the other 254 can be audited against
