...
It joins the old Hugo site against the new one, and the old half is gone: it
reads `../docs/sitemap.xml` and `../content/{post,publication}/`, all deleted at
the migration. `legacy_map.json` is its committed output and is what
`generate_pages.py` reads. This is kept as the record of how the map was built.

To re-derive it, both halves are recoverable, but from different places:
  content/  is in the git history — `git show <last-hugo-commit>:content/...`
  sitemap   was never tracked (the root .gitignore had `docs/*`); the only copy
            was the deployed one on the `gh-pages` branch, so it is committed
            here as `legacy_sitemap.xml` — 354 distinct URLs, the complete list
            of what the old site published. `legacy_map.json` maps 100 of them;
            the other 254 are deliberately unmapped (see CLAUDE.md), and this
            file is the only thing that decision can be audited against.
