...
| `news/2026-rolling-the-bones/featured.jpg` | **used** — the shaman reading burnt bones, and the source is **square** (526×526), which neither crop survives raw: the hero keeps only the middle ~77% of the height and would take the head and the bones with it. Centred instead at its native 526px on a 1248×702 plate of the photograph's own border black `#171717` — not the cream the other plated heroes use, because here the plate abuts a dark picture rather than the panel, and at that colour the seam is invisible. 75% of the plate height, so the subject clears the hero crop with ~9px to spare at the worst case (a 920px panel). 52 KB. **JPEG and not WebP because this file is also the `og:image`**, and not every social crawler renders a WebP or an AVIF card |
| `news/2026-rolling-the-bones/hunters.webp`, `minima.webp` | **used** — the deer hunt and the local/global minimum surface, both floated right at `width: 35%` of the 40rem prose column, so ~224px shown and 480px encoded. 45 and 25 KB. **A float inside `.news-prose` only wraps the paragraph it is in** — that sheet is `display: grid`, and a float cannot escape its own grid item, so each of these sits inside a `<p>` at least as tall as the picture. `minima` was moved off the two-line paragraph that introduces it for exactly that reason: it left a 115px gap instead of wrapping |
| `news/2026-rolling-the-bones/attractors.webp` | **used** — the two attractors and the trajectory crossing between them, 1100×600 / 98 KB. A full-width `<figure>` rather than a third float, because it carries the post's one figcaption |
...
| `news/2025-sam-marine-nhs-prize/featured.png` | **used** — the NHS wordmark, white on NHS Blue `#005eb8` at 1400×788 / 36 KB. Rendered from Wikimedia Commons' `National_Health_Service_(England)_logo.svg` (public domain as simple text and shapes; the mark itself is a UK trademark, so this is editorial use — see NOTICE.md). There is no rasteriser or Node on the machine this was built on, so the three glyph paths were flattened with a throwaway Pillow script rather than converted; the SVG is 1.2 KB and re-renderable from the same source if the plate ever needs another size. **There is a rasteriser on this machine after all**: headless Chrome. Size `html,body` to the plate in an HTML file, inline the SVG, then `chrome.exe --headless --disable-gpu --hide-scrollbars --force-device-scale-factor=1 --window-size=1400,788 --screenshot=out.png file:///…/plate.html`. Cheaper than flattening paths by hand, and how the first hero for `2024-open-science-sustainability-debate` was made, before the BNA's own graphic replaced it |
| `news/2024-open-science-sustainability-debate/featured.jpg` | **used** — the BNA's own announcement graphic for the debate, and the source is **square** (1080×1080), which survives neither crop raw: the hero keeps the middle 77.4% of the height and the card takes 16:9, which of a square is 56%. Centred instead at 591px on a 1400×788 plate of the section cream `#e9f2ee` — 75% of the plate height, the proportion `2026-rolling-the-bones` already uses, which leaves the poster's outer edge ~5px inside the hero's cut at the worst case (an 808px hero box, measured) and its cream card ~22px inside. Cream and **not** the graphic's own border red: a flat red against the source's *textured* red drew a visible seam, and the poster's red frame gives it a crisp edge on cream anyway. q85 at **4:4:4** / 91 KB — 4:2:0 saves 25 KB and softens the green and red text, which is most of the picture. The 1080 original is beside it as a gitignored `featured_full.jpg` |
| `news/2026-university-after-ai/featured.gif` | **used** — the debating-society photograph with the post's title bleeding down it, 600×394 / 244 KB, 23 frames on a 2.8s loop, built by `make_hero.py` beside it. **600px is the whole of what the source had**, and the budget is a ceiling, so it is not upscaled. There is no melted-horror face on this machine and no licence to ship one, so the melt is procedural — the title is set in Impact, its contour displaced by two smooth noise fields, and each drip grown downward off the bottom contour to a terminal bead, all at 4x and downsampled, which is what stops a roughened edge aliasing into fringes. Three things this file is the record of: **a drip is a spec, not a drawing**, so one number per frame (`grow`) is the whole animation and the poster is simply one value of it; **the dark ground is painted once and held**, because recomputing a 17px-blurred halo per frame changes almost every pixel around the title on every one of them and cost 654 KB against 366 KB; and **64 colours, not 255**, which on this picture is not tellable apart at 4x because the film grain dithers it already, and saves a further 122 KB. **The title lives in the middle 58% of the height**, because that is all the reader's hero keeps of a 1.52:1 picture at a 920px panel — measured in the browser, not assumed; the script prints the clearance for both crops on every run (+70px card, +16px hero). `--still` writes the same frame as a JPEG if a still is ever wanted back, but **only one `featured.*` may exist**, or the hero becomes whichever extension `FEATURED_EXTENSIONS` lists first. The source photograph is beside it as a gitignored `featured_full.jpg` — **its provenance is unestablished**, so settle that before the post leaves draft |
| `news/2026-evidence-accumulation-widget/featured.gif` | **used** — cogmod's widget capture with a spinning cut-out reaction face, a red arrow and the cogmod hex on it, built by `make_thumbnail.py` beside it. It takes its three sources as arguments — a path **or an http(s) link** it caches locally — so only `reactface.mp4` is committed, being the one input no other repository has; the capture (`cogmod/man/figures/anim_widget.gif`) and the logo come from the cogmod checkout, the capture cached as a gitignored `featured_full.gif`. **820×461 / 134 frames / 1.9 MB**, and that last number is the thing to read before touching this file. Four things it is the record of. **The capture is cropped to the card's own 16:9 first**, which is what makes a corner of the file a corner of the picture anyone sees: uncropped, the reader's hero keeps only the middle 52% of the height and the card 67%, so a logo in the true top-right corner is in neither. Cropped, the card shows the whole file and the hero still keeps 78% — and nothing is lost that the site ever showed, the crop dropping the title and model tabs off the top and the assumptions row off the bottom. **A spinning sticker is bounded by its diagonal**, so the face is scaled from the sweep radius, not the other way round. **A constant spin costs about three times the file, and there is no way round it**: a large photographic region changing on every frame is the one thing a GIF cannot encode cheaply — 6.4 MB at full frame rate, 1.9 MB after every lever below, against **709 KB for the same composition with the spin as a 1.6s burst**, which is what this was before someone asked for the constant one. The levers, measured: `FRAME_STEP = 2` (every 2nd frame at twice the delay, same 26.8s) halves it outright; **dropping the sticker's drop shadow is worth 26%** (2921 KB → 2167 KB), because a soft ramp around the whole perimeter re-quantises differently on every frame where on a still sticker it is nearly free; and the palette is down to 48. `dither=Image.NONE` throughout, worth 821 KB against 530 KB when this was still. **The face passes behind the logo** by draw order rather than by geometry. `--still` writes a gitignored poster with the hero band and the sweep circle drawn on it |
| `news/2026-evidence-accumulation-widget/logo.webp` | **used** — the same cogmod hex again, in the body floated right at `width: 20%` of the prose column. 260×300 for a 128px display, and **WebP not PNG: 14 KB against 80 KB** for the identical picture with its alpha intact — flat neon on a dark ground is exactly what lossy-with-alpha is good at. The paragraph it floats in measures 162px against the picture's 148px, which is the margin the whole float depends on: `.news-prose` is a grid, a float cannot escape its own grid item, and a picture taller than its paragraph leaves a column-wide gap rather than wrapping — see [news.md](news.md) |
| `news/2023-ohbm-brainart/featured.jpg` | **used** — the exhibition stand at OHBM 2023, cropped to the card's own 16:9 out of `memories/img/2023_dom_ohbm.jpg`. **That memory photo is the source of record** (2000×1500, already in the repository), so there is no `_full` beside this one — re-crop from the memory if it ever needs another size. 1400×788 / 161 KB. The crop is y 0–1125 of the source and **not centred on the frame**: the reader's hero keeps only the middle 77.4% of a 16:9 picture at a 920px panel (measured, not assumed), and the subject band runs from the top of Dom's head to the foot of the two canvases — so the crop centres *that*, which leaves +34px above the head and +41px below the canvases. Cropped to 16:9 first, the card shows the whole file. **JPEG and not WebP because this file is also the `og:image`** |
| `news/2023-ohbm-brainart/attractor.webp` | **used** — the same EEG segment drawn twice, as a time series and as a state-space trajectory, lifted from the proposal that was submitted to the BrainArt SIG. 1100×457 / 23 KB, a full-width `<figure>` in the prose column. WebP against **40 KB as JPEG and 41 KB as a 64-colour PNG** — flat line art on white is the case where the alternatives both lose |
| `news/2023-ohbm-brainart/catalogue.webp` | **used** — the 2023 Brain Art Exhibition's own catalogue page, cropped to the whole of the lab's entry: the banner, the artist block, the statement, the two attractors and the explanatory panel, down to the rule that closes it before the next artist. It is in the post as evidence rather than as reading matter — an outside page listing the work carries what a paragraph paraphrasing the artist's own pitch cannot. 1100×1367 / 99 KB, full width in the prose column, so ~795px tall on screen: **tall on purpose, and the one figure here that is not sized to be read**. **Captured with headless Edge, not the preview pane**, which caps a screenshot at its own width (~800px): `msedge.exe --headless=new --hide-scrollbars --force-device-scale-factor=2 --window-size=1440,2600 --virtual-time-budget=12000 --screenshot=<abs path>`, then cropped and downsampled to 1100 — 2× then down is what keeps the small type crisp. Four things it is the record of. `--screenshot` **needs an absolute path**, or it fails with "Access is denied" and writes nothing. **Google's cookie notice is `position: fixed` at the bottom-left of the viewport**, so the window is captured far taller than the crop needs and the notice lands below it — nothing had to be clicked or dismissed, and a window sized to the crop would have put the notice *inside* it. **The cut is measured, not eyeballed**: an artist's block is a full-width band of the site's dark green, so the next artist is found by scanning rows for that colour and the crop lands in the gap above it. And **the video embed's right edge is clipped in the picture because the page itself clips it** — that is what a visitor sees, so it was left alone. Re-shoot if the page changes; there is nothing here to regenerate it from |
| `news/2023-ohbm-brainart/sketch.webp` | **used** — the pencil sketch of the display proposal, from the same submission. Shown at `max-width: 55%` of the prose column (352px), so 700×782 encoded / 46 KB, against 78 KB as JPEG. A centred `<figure>` and **not a float**: it is portrait, and a float only wraps the paragraph it sits in — no paragraph in this post is the 393px tall it would need. The originals of both figures live outside the repository, in the OHBM submission, so neither is regenerable from anything here |
| `news/2024-breaking-barriers-reproducibility/featured.jpg` | **used** — a screenshot of the Sussex page the post links to, taken by `make_hero.py` beside it, which is the only thing that can regenerate it: re-run it if that page is restyled. 1400×788 / 132 KB, q88 at **4:4:4**, because most of the picture is small text and the Sussex green. Playwright at a 1600px viewport and `device_scale_factor=2`, then down — 2× then down is what keeps the type crisp, the same recipe `2023-ohbm-brainart/catalogue.webp` records for headless Edge. Three things it is the record of. **The cookie banner is hidden from the outside** (`#cookie-options { display: none }`), not answered: it is the first element on the page and pushes the layout down ~320px, and hiding it records no consent either way. **The cut is at the photograph's bottom edge**, one line above the page's own caption, which misspells the name. And **the capture is 988×560 centred on a cream plate rather than filling it**: the reader's hero is 823px wide at every viewport width from 1280 up (measured — the `max-height: 22rem` is what pins it), so it keeps 76% of the height and cuts 95px off each end, which would take the Sussex header and the photograph with it. The plate's 114px band clears that by 19px. **JPEG and not WebP because this file is also the `og:image`** |
