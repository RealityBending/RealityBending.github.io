...
**Steps 1–4 are done and committed.** `WIP/` is gone, the Hugo site is deleted,
the site is at the repository root, and everything §4 asks for was verified
locally: 148 content pages, 100 redirect stubs, 147 sitemap URLs;
`check-paths.py` PASS over 149 routes; `check-routes.html` PASS over all 18
route shapes; every request on the homepage 200, no console errors beyond
Altmetric's own 404s for DOIs it has no record of.

**What is left is yours, because it is not a commit:**

- **§5 — switch *Settings → Pages → Source* to GitHub Actions.** This is the
  one-way door. Until you do it the live site is still `gh-pages` and nothing
  the migration commit did is visible to anyone.
- **§6 — delete `gh-pages`**, but only after §5 is confirmed live.
- **§7 — the after-it-is-live checks.**
- **§8 — the abstract question**, still open.
...
## 3 · Freeze `tools/build_legacy_map.py` — done
...
**One correction to what this file used to say here.** The old sitemap is *not*
in the git history — the root `.gitignore` had `docs/*`, so `docs/sitemap.xml`
was never tracked and `git show <last-hugo-commit>:docs/sitemap.xml` fails. The
only copy was the deployed one on `gh-pages`, which §6 tells you to delete. It
is therefore now committed as `tools/legacy_sitemap.xml` — 354 URLs, the
complete list of what the old site published. The other half of the join, the
Hugo `content/` tree, *is* in the history and is recoverable normally.

---

## 4 · Verify locally, before pushing — done
...
- **Delete the `gh-pages` branch** once the new site is confirmed live. Leaving
  ...
  and it will confuse the next person. The one thing on it that was not
  recoverable elsewhere — the old `sitemap.xml` — has been copied to
  `tools/legacy_sitemap.xml`, so nothing is lost with it.
